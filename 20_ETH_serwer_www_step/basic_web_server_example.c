...
//rozdzielczosc kroku dla steppera
...
//        uint8_t step_x=0;
        ...
        if (step_cmd==6){
        	...
        	if(find_key_val(str, gStrbuf,5,"ox")){
        		...
        		lcd_int(step_x);
        		return(6);
        		/*for(step_left=0; step_left < step_x; step_left++){
        			if(ms2_flag){
        			kroki_lewo();
        			ms2_flag=0;
        			}
        		}*/
        		/*if( ms2_flag && step_left < step_x ){
        			lcd_locate(1,0);
        			lcd_str_P( PSTR("dupa") );
        		   kroki_lewo();
        		   ms2_flag=0;
...
/*STEPPER MOTOR INIT SECTION*/
		...
		//OCR0 = 31;							/* dodatkowy podzia³ przez 31 (rej. przepe³nienia) */
		//OCR0 = 78;							//przepelnienie dla 200Hz
		OCR0 = 39;							//przepelnienie dla 400Hz IDEANE KROKI!
		//OCR0 = 22;						//przepelnienie dla 700Hz
...
/*END OF STEPPER MOTOR INIT SECTION*/
...
/*LCD INIT SECTION*/
		...
		lcd_str_P( PSTR("Stos TCP/IP OK!") );	// napis z pamiêci FLASH
		lcd_locate(1,0);
		lcd_str_P( PSTR("Robert Mleczko") );			// napis z pamiêci FLASH
